    ...
    public sealed partial class CaptureImagePage
    {
        ...
        protected override void OnNavigatedTo(NavigationEventArgs e)
        {
            if (_isPreviewing == false)
            {
                ...
                captureButton.Content = "capturar";
            }
            else if (_isPreviewing == true)
            {
                CleanCapture();
            }
        ...
        {
            if (captureButton.Content != null && captureButton.Content.ToString() == "capturar")
            {
                captureButton.Content = "cancelar";
        ...
        {
            Frame.Navigate(typeof (NovaPostagem), _bitmap);
        }
        ...
        /// </summary>
        private async void InitializePreview(bool traseira = true)
        {
        ...
        private async void StartPreview()
        {
            ...
            previewElement.Source = _captureManager;
        ...
        private async void SetFocus(uint? focusValue = null)
        {
            ...
            try
            {
                ...
                if (_captureManager.VideoDeviceController.FocusControl.Supported)
                {
                    ...
                    _captureManager.VideoDeviceController.FocusControl.Configure(new FocusSettings() { Mode = FocusMode.Manual, Value = focusValue, DisableDriverFallback = true });
        ...
        private async void CaptureFifteenByNineImage()
        {
            ...
            using (var imageStream = new InMemoryRandomAccessStream())
            {
                ...
                nomeFoto = "fotogram_" + DateTime.Now.Ticks.ToString() + ".jpg";
                StorageFile capturefile = await folder.CreateFileAsync(nomeFoto, CreationCollisionOption.ReplaceExisting);
        ...
        private async void CapturePreviewWithoutMondifications()
        {
            ...
            StorageFile capturefile = await ApplicationData.Current.LocalFolder.CreateFileAsync("fotogram_" + DateTime.Now.Ticks.ToString(), CreationCollisionOption.ReplaceExisting);

            ////take & save photo
            await _captureManager.CapturePhotoToStorageFileAsync(format, capturefile);
        ...
        private async void LoadCapturedphoto(string filename)
        {
            ...
            _bitmap = new WriteableBitmap((int)imgProp.Width, (int)imgProp.Height);
            await _bitmap.SetSourceAsync(savedPictureStream);
            takenImage.Source = _bitmap;
            takenImage.Visibility = Visibility.Visible;
